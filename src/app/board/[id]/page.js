...
export default function SelectedBoard() {
  ...
  const fetchBoardPermission = async () => {
    try {
      ...
      if (
        loggedUserId.value == parentProject?.associated_user_id
      ) {
        setPermissionType("write");
      }
    } catch (error) {
  ...
  const onDragEnd = async (result) => {
    ...
    const sourceColumnIndex = columns.findIndex(
      (col) =>
        col.column_id.toString() === source.droppableId
    );

    const destinationColumnIndex = columns.findIndex(
      (col) =>
        col.column_id.toString() === destination.droppableId
    );
  ...
  const updateBoardName = async () => {
    ...
    try {
      await callApi(
        `${serverUrl}/boards/${id}/update-name`,
        "PUT",
        {
          name: boardName,
        }
  ...
  const deleteBoard = async () => {
    if (permissionType !== "write") {
      alert(
        "No tienes permisos para eliminar este tablero."
      );
    ...
    try {
      ...
      alert(
        "Hubo un error al intentar eliminar el tablero."
      );
    }
  };
